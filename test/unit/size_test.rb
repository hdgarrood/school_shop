require 'test_helper'

class SizeTest < ActiveSupport::TestCase
  def test_attr_readers
    sr = Size.new(8, 9, 'years')
    assert_equal(8, sr.lbound)
    assert_equal(9, sr.ubound)
    assert_equal('years', sr.unit)
  end

  def test_conversion
    sr = Size.new(8, 9, 'years').convert_to('height_cm')
    assert_equal(Size.new(128, 134, 'height_cm'), sr)
  end

  def test_emptiness
    assert(Size.new(nil, nil, nil).empty?)
  end

  def test_non_emptiness
    assert(!Size.new(6, 7, 'years').empty?)
  end

  def test_has_range
    sr1 = Size.new(110, 110, 'height_cm')
    sr2 = Size.new(110, 113, 'height_cm')

    assert(!sr1.has_range?)
    assert(sr2.has_range?)
  end

  def test_to_s_for_no_range
    sr = Size.new(8, 8, 'years')
    assert_equal('8 years', sr.to_s)
  end

  def test_to_s_for_inches
    sr = Size.new(12, 13, 'collar_inches')
    assert_equal('12-13"', sr.to_s)
  end

  def test_to_s_for_cm
    sr = Size.new(98, 104, 'height_cm')
    assert_equal('98-104cm', sr.to_s)
  end

  def test_to_s_for_years
    sr = Size.new(5, 6, 'years')
    assert_equal('5-6 years', sr.to_s)
  end

  def test_construction_from_hash
    sr1 = Size.new(5, 6, 'years')
    sr2 = Size.new(:lbound => 5, :ubound => 6, :unit => 'years')

    assert_equal(sr1, sr2)
  end
end
